...
- [ ] 1. 建立项目隔离、版本锁定的 Windows 工具链与 workspace（AC1）
  ...
  - [ ] 使用项目隔离的官方 Node.js 24 LTS，不修改用户/系统 PATH；记录实际验证版本。根 `packageManager` 精确锁定 pnpm，禁止 `latest` 留在持久配置或锁文件生成脚本中。
  - [ ] 初始化 `apps/windows` 的 React + TypeScript + Vite 工程和 `src-tauri`；锁定 Tauri core `2.11.5`、`@tauri-apps/cli 2.11.4`，提交 Rust/pnpm 锁文件变化。
...
- [ ] 2. 建立最小 Tauri 宿主和严格平台边界（AC1）
  - [ ] 创建 `apps/windows/src-tauri/src/{main.rs,lib.rs,commands/}`；仅注册批准的健康与合同探针 commands，不暴露 SQL、文件、shell、任意 URL/HTTP 或反射式 command。
  - [ ] Tauri command 直接调用 `radar-core`/既有安全适配逻辑，不在宿主复制业务规则；保持 Rust panic containment 和稳定 `AppError` 映射。
  - [ ] 配置严格 CSP 和最小 capabilities；生产配置只加载打包资源，不保留开发服务器地址、调试 capability 或宽权限插件。
...
- [ ] 3. 建立 `DesktopApi` 单一 UI 入口（AC1、AC2）
  ...
  - [ ] 为 health、validation/internal error、effect first/repeat/conflict 与受控 secret probe 提供显式方法；禁止通用 `execute`/`invokeAny` API。
...
- [ ] 5. 建立 Windows 契约往返和幂等验证（AC2）
  ...
  - [ ] TypeScript contract tests 使用 `contracts/fixtures/golden` 作为唯一 fixture 来源；不得在 `apps/windows` 复制或手写第二份契约真相。
...
- [ ] 6. 建立 Windows 安全存储—短租约测试链路（AC2；NFR15/NFR20 派生约束）
  ...
  - [ ] 使用测试专用 fake/受控入口生成 runtime canary；不得把 canary 写入 fixture、源文件、前端 store、IPC 响应、快照或普通错误信息。
...
- [ ] 7. 演进 Story 1.1 工程边界门禁并建立完成证据（AC1、AC2）
  ...
  - [ ] 运行项目隔离 Windows 门禁：frozen pnpm install、format/lint/typecheck、Vitest/Testing Library、Vite production build、Tauri Rust tests/build 与真实启动 smoke。
