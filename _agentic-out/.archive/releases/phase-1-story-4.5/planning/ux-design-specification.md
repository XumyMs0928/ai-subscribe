...
lastReviewedAgainst: '_agentic-out/planning/prd.md'
lastReviewedAt: '2026-08-12'
revisionHistory:
  - date: '2026-08-17'
    reason: '批准第一阶段 Windows RSS 最小闭环，收敛可见导航、核心旅程、状态与延期入口'
  - date: '2026-08-12'
    reason: '对齐已验证的 FR64/NFR54，补充 GitHub 条件化项目发现、命中解释、自动关注控制、身份去重与发现故障隔离体验合同'
  - date: '2026-08-12'
    reason: '对齐已验证的 FR59–FR63 与 NFR33/NFR35/NFR52/NFR53，补充同步结果、处理状态、固定引导入口、配置校验分流和 Windows 单实例体验合同'
  - date: '2026-08-12'
    reason: '对齐最新版跨平台 MVP、设备本地通知、设备数据边界与受控移动测试分发要求'
---
...
## 第一阶段 UX 覆盖：Windows RSS 最小闭环

本阶段只交付 Windows 上可完整走通的 RSS 垂直旅程。一级可见入口收敛为“情报”“雷达规则”“同步结果/状态”“设置”；GitHub Release、arXiv、GitHub 项目发现、AI、通知、反馈、全文搜索、收藏、托盘和移动端入口不显示不可用占位，也不得显示假成功。

第一阶段主旅程固定为：配置关注规则与 RSS/Atom 来源 → 手动同步 → 查看新增/更新/跳过/失败计数及最小结果 → 浏览经规范化、去重和透明规则分流的主情报 → 查看事实、规则依据与溯源 → 用系统浏览器安全打开原文。界面必须覆盖加载、运行中、成功但零结果、成功且有结果、部分/完全失败和重试状态；已有本地内容可在无新网络请求时读取，但不据此宣称完整离线 Epic 已完成。

详情页若保留 AI 区域，只能明确显示“本阶段未启用/未配置”，不得生成演示 AI 结论冒充真实分析。Windows 继续满足键盘、可见焦点、非颜色状态、响应式缩放和错误恢复合同；当前不扩大为盲人屏幕阅读器承诺，真实系统主题/缩放矩阵留在隔离发布环境。Apple、iPhone、iPad 和 Android 的完整 UX 规范原文保留为后续阶段目标。
...
- 可用性基线：Windows 支持 Radix 语义与键盘；Apple 平台支持 Dynamic Type、平台焦点与系统外观；Android 支持系统缩放、平台焦点与 Compose Semantics。当前产品不承诺盲人屏幕阅读器支持。
...
- Windows 与平板端支持键盘、鼠标或触控板；移动端核心任务支持触控及适用的平台输入方式。
...
- SwiftUI 能原生处理 iPhone/iPad 尺寸类别、NavigationSplitView、Dynamic Type、平台焦点和系统外观。
...
- 是否保留用户输入、选择和滚动上下文？
- 是否说明错误影响范围和恢复动作？
- 是否支持 Windows 键盘、Apple Dynamic Type、Android 系统缩放及各平台焦点恢复？
...
- 核心旅程支持仅键盘完成。
- 支持键盘、Windows 高对比度和 100%–200% 缩放；真实系统矩阵只在专用发布测试机或隔离虚拟机执行。
- Radix 覆盖层正确管理焦点进入、循环、Esc 关闭与恢复。

**iOS/iPadOS**

- 支持 Voice Control、Switch Control 与 Full Keyboard Access；当前产品不承诺盲人屏幕阅读器支持。
...
- 支持 Switch Access、Voice Access、字体与显示缩放；当前产品不承诺盲人屏幕阅读器支持。
...
- 键盘、高对比度、减弱动画；100%–200% 真实系统缩放矩阵在专用发布测试机或隔离虚拟机执行。
...
- Voice Control、Switch Control、Full Keyboard Access 与 Dynamic Type。
...
- 横竖屏、多窗口、键鼠、触控与手写笔。
- Switch Access、Voice Access、字体与显示缩放。
...
- 自动检查不能替代真实设备上的动态字体、显示缩放、键盘、触控、焦点、高对比度和减弱动画测试；当前产品不设置盲人屏幕阅读器运行门禁。
...
- 真实实现必须分别通过适用平台的键盘/触控、焦点、动态字体或显示缩放、高对比度和减弱动画验收，不以 HTML 原型替代运行时验收；当前产品不承诺盲人屏幕阅读器支持。

正式评审记录：`_agentic-out/reviews/2026-08-10-ux-visual-quality.md`。

## 2026-08-12 PRD 对齐增补

本节是对已完成 UX 规格的追加式修订。原有视觉基础、信息架构、核心旅程、组件策略与响应式规则继续有效；以下条款补齐最新版 PRD 的产品边界，并在发生歧义时优先于旧文中的概括性描述。

### 完整产品来源完成度（当前阶段延期）

- RSS/Atom、GitHub Release、arXiv 是完整产品阶段的三类必交付来源，届时均须出现在来源管理、同步状态、失败诊断、独立重试和验收路径中。
- 当前第一阶段只交付 RSS/Atom；GitHub Release、arXiv 不显示为失败或阻塞。三类来源任一缺失时，界面不得将产品标记为“完整产品三来源已就绪”。
- 来源总览应分别显示类型、启用状态、最后成功同步时间、退避/失败状态和下一步动作；不得用单一汇总成功状态掩盖某一类来源尚未交付或持续失败。

### 设备本地通知与后台时效边界

- MVP 不使用远程推送服务。只有当前设备已在本地完成来源同步、重大动态判定，并具备完整溯源、达到用户阈值且已获通知权限时，才向操作系统提交通知。
- 移动端不得使用“实时推送”“始终后台运行”或“固定时刻送达”等承诺。同步状态应展示最后成功时间、后台受限原因和前台手动同步入口。
- 应用恢复执行后，仍具提醒时效的重大动态可以通知；已经失去即时性的内容仅进入主情报流，不补发带有“刚刚发生”含义的通知。
- 通知权限只在用户理解重大提醒价值且首次确有发送需要时请求。拒绝或撤销后保留采集、浏览、搜索、溯源、反馈和前台同步，并提供设置入口，不重复强迫授权。

### 当前设备数据边界与删除后果

- 首次使用、设置、诊断与数据管理界面必须明确说明：配置、收藏、反馈、凭据和历史情报只保存在当前设备；MVP 不提供云端备份、跨设备同步或自动迁移。
- 涉及“仅当前设备”的操作使用清晰的作用域文案，例如“删除此设备上的正文”“仅修改此设备的来源设置”，避免账号级或跨设备暗示。
- 卸载、清除应用数据、删除缓存、删除收藏正文、移除凭据或执行无法回退的数据操作前，确认界面必须逐项说明将删除和允许保留的内容，以及是否可恢复。
- 删除完成后的搜索、详情、收藏、诊断和网络请求不得继续暴露或使用已删除正文、索引或凭据；成功反馈应说明实际清理范围。

### 受控移动测试分发与隐私披露

- iOS/iPadOS 与 Android 候选构建只有在签名/安装资格有效时才能进入受控测试分发；资格无效时提供具体原因与修复路径，不展示为普通安装失败。
- 移动候选构建的分发入口或随附说明必须完整覆盖七类信息：设备本地数据、系统权限、访问的公开来源、发送给第三方 AI 的数据、正文保留与删除、诊断信息范围、无云备份或跨设备同步。
- 隐私说明必须在首次向第三方 AI 发送内容前可访问，并在设置与分发说明中可再次查看；界面声明必须与实际权限、网络请求和数据保留行为一致。
- 测试分发状态不是日常内容状态。它应位于构建/关于/诊断上下文，不进入主情报流，也不使用高优先级情报的颜色或图标语义。

### 对现有 UX 产物的影响判断

- 现有设计令牌、视觉方向、桌面/手机/平板布局和核心浏览交互无需改变。
- `DESIGN.md` 只需补充数据作用域、通知时效和分发状态的语义约束；不新增颜色体系。
- `EXPERIENCE.md` 需要补充来源完成度、通知失效降级、当前设备数据删除确认，以及受控测试分发失败的状态转移与验收路径。
- HTML 交互原型仍是体验验证样品，不代表三类真实来源、原生通知、移动签名或隐私清单已经实现。

## 2026-08-12 最终 PRD 验证对齐增补

本节对齐已通过编辑后验证的 FR59–FR63 及对应 NFR 门禁。若本节与旧文中的概括性路径、状态或校验描述不一致，以本节为准；既有设计令牌、平台组件基础、响应式策略和核心信息架构继续有效。

### 同步结果最小消费闭环（FR59 / NFR52）

- “同步与诊断”必须同时提供最近一次全部来源同步和单个来源同步的结果摘要，按 RSS/Atom、GitHub Release、arXiv 分别显示新增、更新、跳过和失败数量。
- 摘要下方提供本次成功转换的最小只读结果列表；每项显示来源类型、发布方、原始标题、来源可提供时的发布时间、采集时间和原文链接。
- 最小结果列表在无 AI Key、AI 待配置、评分未完成、搜索尚未建立或完整详情不可用时仍可打开；这些增强状态不得遮挡同步成果。
- 零结果使用“本次没有新增或更新”状态，不使用成功空白页；部分成功同时显示成功结果与失败来源，禁止用全局错误替换已成功内容。
- 来源失败操作附着在对应来源行，允许独立重试；处于退避期时显示原因和可重试时间。

### 用户处理状态与收藏独立性（FR60 / NFR53）

- 每条情报具有且仅具有“未查看”“已判断”“待研究”之一；新入库默认“未查看”。列表浏览、选择条目或打开详情不得自动改变处理状态。
- 处理状态使用显式菜单、分段控件或等价原生操作修改，并提供当前值、目标值和撤销反馈；不得把“已查看”作为隐式第四状态。
- 收藏与处理状态并列但独立。收藏、取消收藏、删除正文快照以及 AI 状态变化均不得改变用户处理状态。
- 列表筛选同时支持处理状态和收藏状态，两个维度使用不同标签、图标、无障碍名称和筛选分组。
- 应用重启、离线恢复、移动挂起或进程回收及受支持版本升级后，六种“处理状态 × 收藏状态”组合必须保持不变。

### 固定配置引导入口（FR61 / NFR33）

- 首次启动始终先展示可浏览的主情报流；渐进配置不得作为阻塞式启动门、全屏强制向导或关闭后反复弹出的模态流程。
- “设置”根级页面必须包含名称固定为“配置引导”的可见入口。入口不得仅存在于搜索、命令面板、Tooltip、帮助文档或深层子页面。
- 从主情报流开始，到达“配置引导”入口不得超过两次用户发起的导航操作。推荐平台路径：Windows `主情报流 → 设置 → 配置引导`；iPhone/Android 手机 `主情报流 → 设置标签 → 配置引导`；iPad/Android 平板使用等价的设置根级入口。
- 入口显示未开始、进行中、已跳过、部分完成或已完成状态；继续时回到首个未完成步骤，而不是重新开始。
- 每个可延后步骤均可跳过；退出、系统返回、窗口关闭、应用挂起或进程恢复不得丢失已保存配置，也不得阻断主情报流、同步、搜索和详情。
- 稳定可见标签和自动化名称使用“配置引导，{状态}”；仅靠进度环或颜色不得承担状态表达。

### 配置校验双通道（FR62）

`RuleImpactPreview` 和所有赛道、关键词、排除词、来源、阈值保存流程必须把“阻断性无效配置”与“过窄配置风险”设计成两个不同通道。

**阻断性无效配置：**

- 类别固定为：表达式无法解析、数值或日期越界、范围下界高于上界、来源地址或标识无效或协议不受支持。
- 保存动作保持不可用或提交后被阻止；错误就近关联受影响字段，同时显示所属无效类别和具体修正方式。
- 首个阻断错误获得焦点；修正后重新校验，不弹出风险确认对话框。

**过窄配置风险：**

- 仅用于格式和逻辑有效但会停用全部来源，或会把当前设备可评估的全部高可信候选过滤的配置。
- 保存动作仍可继续，但先显示受影响条件、可能漏报后果和“返回修改 / 理解风险并保存”两个明确选择。
- 风险确认不得使用破坏性红色语义，也不得暗示未来一定漏报；用户确认后保存，并说明从下一轮同步生效、历史情报不被改写。

**无风险配置：**

- 直接保存，不显示额外确认；不得为所有配置统一增加一次无意义确认。
- 验收预览须覆盖四类阻断性无效配置各至少一个固定样本，以及“全部来源停用”“全部高可信候选被过滤”两类过窄风险样本。

### Windows 单实例体验（FR63 / NFR35）

- 在同一 Windows 用户会话中，再次启动应用不得创建第二个可交互窗口、第二套导航状态或重复的后台任务入口。
- 若主窗口可见，再次启动必须将现有窗口恢复、置前并聚焦到可操作区域；不得重置当前筛选、选中项、详情滚动位置或未提交输入。
- 若主窗口已隐藏至托盘，再次启动必须显示并聚焦现有主窗口；托盘状态、同步任务和本地数据上下文继续沿用。
- 若现有窗口最小化或位于其他虚拟桌面，应用应使用 Windows 允许的前台激活行为恢复现有窗口；操作系统拒绝抢占焦点时，须以任务栏/窗口注意状态提供可发现反馈，不创建替代窗口。
- 重复启动过程默认不显示成功 Toast；只有无法恢复现有窗口时才显示可操作错误，并保持单实例约束。
- 显式退出后再次启动属于新的正常应用会话；不得错误恢复已经退出的旧进程。

### 跨平台验收路径更新

| 场景 | Windows | iPhone / Android 手机 | iPad / Android 平板 |
|---|---|---|---|
| 恢复配置引导 | 主情报流 → 设置 → 配置引导，≤2 次导航操作 | 主情报流 → 设置标签 → 配置引导，≤2 次导航操作 | 主情报流 → 设置根级区域 → 配置引导，≤2 次导航操作 |
| 阻断性无效配置 | 字段错误 + 类别 + 修正方式；阻止保存 | 原生表单错误并聚焦首项；阻止保存 | 等价表单错误；宽屏可同时显示预览 |
| 过窄配置风险 | Alert Dialog 或等价集中确认 | Sheet/Alert 或平台等价确认 | Dialog/详情区确认，不丢失表单上下文 |
| 处理状态 | 列表快捷操作与详情显式控件 | 详情或菜单中的显式状态控件 | 列表—详情中的显式状态控件 |
| 重复启动 | 恢复并聚焦现有窗口 | 不适用 | 不适用 |

### 对 UX Spine 与下游产物的同步要求

- `DESIGN.md` 必须补充“配置引导”“用户处理状态”“配置无效/风险”“单实例恢复”所需的语义令牌使用边界和组件状态，不新增独立品牌色。
- `EXPERIENCE.md` 必须补充上述状态机、入口可达性、保存分流、收藏独立性和 Windows 重复启动恢复路径。
- Architecture 必须承接处理状态持久化、同步结果只读投影、配置校验分类契约和 Windows 单实例激活事件；不得由 UX 指定具体 IPC、互斥锁或存储实现。
- Epics/Stories 必须为 FR59–FR63 分配明确验收所有者，并引用 NFR33、NFR35、NFR52、NFR53 及固定配置样本。

## 2026-08-12 GitHub 项目自动发现 UX 增补

本节对齐已通过验证的 FR64 与 NFR54。它补充“用户手动指定仓库”之外的条件化自动发现体验；若与旧文中只描述固定 GitHub Release 关注的概括性条款冲突，以本节为准。既有主情报流、证据详情、视觉令牌及跨平台布局保持不变。

### 信息架构与入口

- “来源”中保留“固定关注的 GitHub 项目”，用于用户已知仓库的手动关注；新增同级入口“GitHub 项目发现”，用于创建和管理条件化发现订阅。两者不得合并成无法辨识来源的单一仓库列表。
- 每个发现订阅显示名称、启用状态、条件摘要、最近完成时间、最近结果数量和当前运行状态；启用、停用和编辑均从订阅行或详情页显式发起。
- 发现结果在订阅详情中展示，并可从“来源”总览的“待审查发现”进入。主情报流只承载项目后续产生的 Release 情报，不把项目发现结果伪装成 Release 或普通文章。
- Windows 使用来源工作区中的列表—详情结构；手机使用来源列表到订阅详情的单栈路径；平板使用列表—详情多窗格。三端使用相同术语和状态含义。
...
1. 订阅名称与关注赛道。
2. 主要语言与 GitHub Topic。
3. Star/Fork 当前门槛。
4. 可选的 Star/Fork 绝对增长或比例增长条件及观察窗口。
5. 组合逻辑摘要与保存操作。

交互合同：

- 不同条件维度之间明确显示“同时满足”，同一维度多个值明确显示“满足任一”；保存前提供可读句子形式的完整组合摘要，不要求用户从控件排列猜测逻辑。
- 条件为空、数值越界、范围倒置或标识无效时，沿用 FR62 的阻断性校验通道；格式有效但过窄时沿用风险确认通道，不为 GitHub 发现建立第三套错误语义。
- 增长条件旁始终解释：“首次有效观测只建立基线，至少两个带时间戳的有效观测值后才能判断增长；起始值为 0 时不使用比例增长命中。”
- 编辑已启用订阅时说明新条件从下一次发现执行机会生效；修改不得删除历史发现依据、既有 Release、收藏、反馈或固定关注状态。
- 停用订阅暂停后续发现，但保留订阅、结果、历史依据与由其他有效关注维持的 Release 监控；重新启用不把历史项目重新标成“新发现”。

### 发现结果与命中解释

每个结果至少展示：

- 仓库名称、所有者、主要语言、Topic，以及可获得的 Star/Fork 当前值。
- 分类标签“新发现项目”或“新符合条件项目”，并提供各自定义的简短解释。
- 命中的订阅条件、实际观测值和观测时间；增长命中同时展示窗口首值、末值与时间范围。
- 当前关注状态：自动关注、自动关注已停用、固定关注，或在当前订阅中已忽略。
- Release 监控状态和最近一次 Release 同步时间；发现状态与 Release 同步状态分区显示。

规则：

- “新发现项目”不得使用“GitHub 新建项目”等可能误导仓库创建时间的文案；辅助说明固定为“首次进入此设备的发现结果”。
- “新符合条件项目”说明为“此前已有观测，本次首次达到当前订阅条件”。
- 多个订阅命中同一仓库时只显示一个项目主体和一个 Release 监控状态，并在“命中依据”中按订阅分别列出理由。
- 仓库重命名或转移时，详情显示当前仓库名称及适用的身份变化说明，不把它展示成第二个项目或新的 Release 监控对象。
- 首次观测尚不能判断增长时显示“增长基线已建立”，不得显示增长成功、增长失败或零增长结论。

### 自动关注与用户控制

符合条件的项目自动进入设备本地 Release 监控。项目详情和结果行提供三个含义严格区分的控制：

- **忽略此订阅中的项目：** 抑制该项目再次作为当前订阅的新发现出现，并停止当前订阅建立的自动关注；操作前说明其他订阅或固定关注不受影响，并提供撤销入口。
- **停用自动关注：** 保留项目、命中依据和历史数据，但暂停仅由自动发现维持的后续 Release 同步；若另有固定关注或其他有效关注，界面必须说明 Release 监控仍会继续。
- **转为固定关注：** 将项目变为不依赖发现条件的用户固定关注；转换成功后不再提供“因条件失配自动停止”的暗示，并保持同一个项目和 Release 监控对象。

固定关注优先级规则必须可见：

- 已手动固定关注的仓库被发现规则命中时，项目状态保持“固定关注”，只追加发现依据，不出现第二个关注开关。
- 项目已转为固定关注后，停用原发现订阅不得停止其 Release 同步。
- 忽略或停用不得删除已保存的 Release、收藏、反馈和其他合法历史数据；任何真正删除历史数据的操作必须使用独立的破坏性确认流程。

### 发现运行状态与故障隔离

发现订阅使用以下互斥结果状态：

- `执行中`：显示当前订阅正在获取或评估项目，不阻塞既有 Release 浏览与同步。
- `成功，有结果`：显示新发现、新符合条件及已知命中数量。
- `成功，零结果`：明确说明本轮已完整执行但没有符合条件的项目。
- `部分完成`：分页或部分响应失败；显示已确认结果、未完成范围、错误归属和重试入口，不得标记为零结果。
- `受限，等待重试`：显示限流原因和最早可重试时间，强制退避期间禁用重复请求。
- `失败，可重试`：显示受影响订阅、失败阶段、已有数据安全状态和恢复动作。

故障体验必须遵守：

- 发现失败只附着在对应发现订阅或本轮发现任务上，不覆盖固定关注和自动关注项目的既有 Release 成功结果。
- “同步与诊断”分别列出 GitHub 项目发现与 GitHub Release 同步；两者使用独立进度、最后成功时间、错误、退避和重试动作。
- 发现部分分页失败、限流或无效响应时，界面不得展示“本次没有符合条件的项目”。
- 发现恢复后原位更新结果，不重新创建项目、不重复 Release、不重复通知，也不把历史结果再次标记为新发现。

### 可访问性与跨平台行为

- 分类、关注状态、发现状态和 Release 同步状态均使用文字与平台语义，不只依赖颜色或图标。
- 结果的稳定可见标签和自动化名称包含“仓库名称、发现分类、关注状态”；“命中依据”展开控件包含依据数量。
- Windows 键盘路径支持在订阅列表、结果列表、命中依据和控制菜单间顺序导航；覆盖层关闭后焦点返回触发项。
- iOS/iPadOS 与 Android 的忽略、停用和转固定关注均提供可见按钮或菜单项，不把滑动手势作为唯一入口。
- 布局在桌面三栏、手机单栈和平板多窗格之间切换时，保留当前订阅、筛选、选中项目、表单输入及滚动上下文。

### 验收场景补充

1. 创建同时包含赛道、语言、Topic、当前门槛和增长条件的订阅，并核对组合逻辑摘要。
2. 展示“新发现项目”“新符合条件项目”和“增长基线已建立”三种不同结果。
3. 同一仓库被手动固定关注及多个订阅命中时，只存在一个项目和一个有效 Release 监控对象，但保留全部命中依据。
4. 分别执行忽略、撤销忽略、停用自动关注和转固定关注，验证关注行为及历史保留。
5. 模拟仓库重命名或转移，验证身份连续且不产生重复 Release 或通知。
6. 注入超时、限流、无效响应和部分分页失败，验证零结果不会误报，且既有 Release 同步继续可见、可用。
7. 在 Windows、iPhone、iPad、Android 手机和平板上完成订阅创建、结果审查和三类用户控制，并通过对应平台输入方式、焦点和自动化语义检查。

### 对 UX Spine 与下游产物的同步要求

- `DESIGN.md` 需要补充发现分类、关注状态、发现运行状态、Release 同步状态的语义边界，以及订阅表单、结果行、命中依据和控制动作的组件状态；沿用现有色彩体系。
- `EXPERIENCE.md` 需要补充订阅创建、修改与启停、完整组合逻辑、首次观测基线、新发现/新符合条件、身份合并、忽略/停用/转固定关注和发现故障隔离状态机。
- Architecture 必须承接规范仓库身份、订阅条件、带时间戳观测、项目关注优先级、发现与 Release 独立任务边界及历史保留，不得由 UX 指定 SDK、存储表或调度实现。
- Epics/Stories 应将 FR64 拆分为订阅管理、基础发现与解释、增长分类、身份去重、用户控制、历史保留和故障隔离等可独立验证故事，并统一引用 NFR24 与 NFR54。
- 当前 PRD 未定义自动关注项目的容量上限或超限策略；UX 不显示虚假的“无限监控”承诺。若后续产品决策引入上限，须追加可见容量、临界状态和超限处理合同后再进入实现。
