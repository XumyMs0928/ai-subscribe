...
- 屏幕阅读器名称使用“配置引导，{状态}”；仅靠进度环或颜色不得承担状态表达。
...
- 每个发现订阅显示名称、启用状态、条件摘要、最近完成时间、最近结果数量和当前运行状态；启用、停用、编辑和删除均从订阅行或详情页显式发起。
...
- 项目已转为固定关注后，停用或删除原发现订阅不得停止其 Release 同步。
...
- 屏幕阅读器结果名称包含“仓库名称、发现分类、关注状态”；“命中依据”展开控件包含依据数量。
...
7. 在 Windows、iPhone、iPad、Android 手机和平板上完成订阅创建、结果审查和三类用户控制，并通过对应屏幕阅读器与输入方式检查。

### 对 UX Spine 与下游产物的同步要求

- `DESIGN.md` 需要补充发现分类、关注状态、发现运行状态、Release 同步状态的语义边界，以及订阅表单、结果行、命中依据和控制动作的组件状态；沿用现有色彩体系。
- `EXPERIENCE.md` 需要补充订阅 CRUD、完整组合逻辑、首次观测基线、新发现/新符合条件、身份合并、忽略/停用/转固定关注和发现故障隔离状态机。
